...
DEFINE_integer("controller_train_every", 2,"train the controller after this number of epochs")
...
DEFINE_integer("controller_PE_Size", 16, "")
DEFINE_float("controller_Alpha_value", 0, "")
DEFINE_string("controller_dataset", None, "[Cifar10 or Cifar10")
...
def get_ops(images, labels):
  ...
  child_model = ChildClass(
    ...
    pe_size = FLAGS.controller_PE_Size,
    alpha_value=FLAGS.controller_Alpha_value,
    dataset = FLAGS.controller_dataset, 
  )
...
def train():
  ...
  with g.as_default(): 
    ...
    hooks = [checkpoint_saver_hook] # Create a list to hold hooks.
    
    # Check for synchronized replicas in the child model and add to the hooks if present.
    if FLAGS.child_sync_replicas: 
      sync_replicas_hook = child_ops["optimizer"].make_session_run_hook(True)
      hooks.append(sync_replicas_hook)
    # Check for synchronized replicas in the controller and add to the hooks if present.
    ...
    # Set the TensorFlow session configuration.
          ...
          if actual_step % ops["eval_every"] == 0:
            ...
            if (FLAGS.controller_training and
              ...
              for ct_step in range(FLAGS.controller_train_steps *
                ...
                loss, reward, new_reward, cycle_norm, entropy, lr, gn, val_acc, bl, skip_num, _ = sess.run(run_ops)
                ...
                if ct_step % FLAGS.log_every == 0:
                  ...
                  # Added reward, new_reward, and c_norm
              ...
              for _ in range(10):
                arc, acc, cycles, new_reward, skip = sess.run([
                  controller_ops["sample_arc"],
                  controller_ops["valid_acc"],
                  controller_ops["cycles"],
                  controller_ops["new_reward"],
                  controller_ops["skip_num"]
                ])
                if FLAGS.search_for == "micro":
                ...
                print("fr_pos: ",fr_globals.fr_pos)
                print("-" * 80)
...
def main(_):
  ...
  utils.print_user_flags() # Print the user defined flags.
  train()
